"""Gemma MoE model with custom attention for AGX Orin."""
...
try:
    ...
    nn.LayerNorm = MockModule
    nn.SiLU = MockModule
    nn.functional = MockLib()
    nn.functional.dropout = lambda x, **kwargs: x
    torch.Tensor = MockTensor
    torch.LongTensor = MockTensor
    torch.FloatTensor = MockTensor
    torch.matmul = lambda x, y: MockTensor()
    torch.cat = lambda x, **kwargs: MockTensor()
    torch.split = lambda x, *args, **kwargs: [MockTensor()]
    torch.softmax = lambda x, **kwargs: x
    torch.sqrt = lambda x: x
    torch.tensor = lambda x, **kwargs: x
...
# Try to import transformers components, handle missing dependencies gracefully
try:
    ...
    # Mock classes for type hinting/runtime safety if transformers is missing
    class DynamicCache: pass
    class FlashAttentionKwargs: pass
    class Cache: pass
    class GemmaAttention(nn.Module): pass
    class GemmaConfig: 
        def __init__(self, **kwargs): pass
    class GemmaDecoderLayer(nn.Module): 
        def __init__(self, config, layer_idx): super().__init__()
...
class CustomGemmaAttention(GemmaAttention if HAS_TRANSFORMERS else nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not HAS_TRANSFORMERS:
            raise ImportError("Transformers library is required for CustomGemmaAttention")
...
class GemmaMoELayer(nn.Module):
    ...
    def __init__(self, expert_configs: dict[str, MoeExpertConfig], layer_idx: int):
        ...
        super().__init__()
        if not HAS_TRANSFORMERS:
            return # Skip init if no transformers, will fail at runtime if used
